...
public abstract class AbstractAction implements Action, Cloneable {
     ...
     * @return     the binding <code>Object</code> stored with this key; 
     *             if there are no keys, it will return <code>null</code>
     * 
     * @see Action#getValue
     */
    ...
    /**
     * Returns an array of <code>Object</code>s which are keys for which
     * values have been set for this <code>AbstractAction</code>, or
     * <code>null</code> if no keys have values set.
     * 
     ...
     * setting a bound property, e.g. <code>setFont</code>,
     * <code>setBackground</code>, or <code>setForeground</code>. Note
     * that if the current component is inheriting its foreground, background,
    ...
    /*
     * Private storage mechanism for Action key-value pairs. In most cases this
     * will be an array of alternating key-value pairs. As it grows larger it is
     * scaled up to a Hashtable. <p>This is also used by InputMap and
     * ActionMap, this does no synchronization, if you need thread safety
     * synchronize on another object before calling this.
     */
    static class ArrayTable implements Cloneable {
        ...
        public Object[] getKeys(Object[] keys) {
            ...
            if (isArray()) {
                ...
                while (counter > 0) {
                    keys[--counter] = enumer.nextElement();
                }
            }
            return keys;
        }
        ...
        /*
         * Returns true if the current storage mechanism is an array of
         ...
         * The data is saved as an integer indicating how many key/value pairs
         * are being archived, followed by the the key/value pairs. If
