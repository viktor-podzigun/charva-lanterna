...
public class DefaultTreeModel implements TreeModel {
    ...
    /**
     * Determines how the isLeaf method figures out if a node is a leaf node. If
     * true, a node is a leaf node if it does not allow children. (If it allows
     * children, it is not a leaf node, even if no children are present.) That
     * lets you distinguish between folder nodes and file nodes in a file
     * system, for example.
     * <p>
    ...
    /**
     * Returns true if node is a leaf. The way the test is performed depends on
     * the asksAllowsChildren setting.
     */
